...
def generate_test_data(user_id=1, count=10000):
    """生成测试数据"""
...
def main():
    with app.app_context():
        ...
        # 生成10000条数据
        records = generate_test_data(user_id=1, count=10000)
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                print(f"✅ 已保存 {total_saved}/{len(records)} 条记录")
            except Exception as e:
                print(f"❌ 保存第 {i+1} 批数据失败: {str(e)}")
                db.session.rollback()
        
        # 查询统计
        total_count = DietRecord.query.filter_by(user_id=1).count()
        print(f"\n🎉 完成！共成功保存 {total_count} 条饮食记录！")
        
        # 显示一些统计信息
        from sqlalchemy import func
        
        stats = db.session.query(
            DietRecord.food_type,
            func.count(DietRecord.id).label('count'),
            func.sum(DietRecord.calorie).label('total_calorie')
        ).filter_by(user_id=1).group_by(DietRecord.food_type).all()
        
        print("\n📊 数据统计（按食物类型）:")
        for food_type, count, total_calorie in stats:
            print(f"  {food_type}: {count}条, 总热量 {total_calorie}大卡")
